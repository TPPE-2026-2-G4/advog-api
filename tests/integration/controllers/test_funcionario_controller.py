...
def test_criar_funcionario_com_email_existente_retorna_erro(client):
    ...
    response_erro = client.post("/funcionarios", json={
...
def test_primeiro_acesso_retorna_funcionario_atualizado(client, nome, email):
    ...
    response = client.patch(f"/funcionarios/{funcionario_id}/primeiro-acesso", json={
        "senha": "senha123"
    })
    assert response.status_code == 200
    assert response.json()["nome"] == nome
...
def test_primeiro_acesso_funcionario_nao_encontrado_retorna_erro(client):
    response = client.patch("/funcionarios/999/primeiro-acesso", json={
        "senha": "senha123"
    })
...
def test_mudar_acesso_funcionario_muda_atividade_e_manda_funcionario_atualizado(client, nome, email):
    response = client.post("/funcionarios", json={
        "nome": nome,
        "email": email
    })
